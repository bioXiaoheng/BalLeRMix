#Jan 30, 2019: Adopted to reformat panPan2 and hg38 alignment for later ancestral calling
#python script to reformat pantro5 sequence alignment onto hg19 coordinates.
...
def match(ch,axtfile, hfile):
	...
	while l != ['']:
		while l[4] not in ptChrs or l[1] != 'chr'+ch:
			#read on
			for meh in xrange(4):
				l=axt.readline().strip().split(' ')
			if l == ['']: break
		#here they're matched
		if l == ['']: break
...
def main():
	ch = sys.argv[1]
  	path = sys.argv[2]
	axtfile = '%s/hg18_to_panTro2/hg38.panTro2.net.axt.gz' % (path)
	hfile = '%s/hg38matchPP2/chr%s.hg18.pT2.txt.gz' %(path,ch)

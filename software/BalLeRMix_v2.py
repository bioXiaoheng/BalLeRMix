...
'''
def getJKprob(m,n,X,k):
    MN_Prob = getMNprobs(m,n,X)
    prob=0; kjCombos={} #construct dictionary to avoid dups
    #more j-linked classes than k-linked classes
    #for k_alcount in range(1,m/2+1):
    for k_alcount in range(1,m+1):
        j_alcount = m-k_alcount
        k_comps = composition(k_alcount,k)
        j_comps = composition(j_alcount,n-k)
        for ks in k_comps:
            for js in j_comps:
                #merge the two combo, without changing orders
                #iterate through all combos of insertion places, insert K into J
                combo_w_rep={}
                for insPos in itr.product(range(j_alcount+1),repeat=k_alcount):
                    inspos=tuple(sorted(insPos))
                    if insPos not in combo_w_rep:
                        combo_w_rep[insPos] = 1
                        #insPos is a tuple
                        K = [0]*m #; J=[0]*m
                        KJ = js[:]; i=0
                        #print 'before:',js
                        for index in range(k_alcount):
                            KJ.insert(insPos[index]+i,ks[i])
                            K[insPos[index]+i] = ks[i]
                            i += 1
                        #print 'after: ',js
                        assert len(KJ) == m 
                        J = [KJ[x] - K[x] for x in range(m)]
                        #print K, J, KJ, js, ks, insPos
                        combo = (tuple(K),tuple(J))
                        if combo not in kjCombos:
                            kjCombos[combo] = MN_Prob[tuple(KJ)]
                            prob += MN_Prob[tuple(KJ)]
                    else:
                        continue
    return(prob / len(kjCombos))
...
return full unnormalized probability
'''
def getFullJKprobs(m,n,X):
	fullProbs = {}
	MN_Prob = getMNprobs(m,n,X)
	#ignore k=0 (assume k be derived allele count)
	for k in range(1,n+1):
		#initiate probability (before division)
		prob=0
		kjCombos = {} #construct dictionary to avoid dups
		#more j-linked classes than k-linked classes
		#for k_alcount in range(1,m/2+1):
		for k_alcount in range(1,m+1):
			j_alcount = m-k_alcount
				...
				for js in j_comps:
					...
					for insPos in itr.product(range(j_alcount+1),repeat=k_alcount):
						...
						if insPos not in combo_w_rep:
							...
							continue
		#finished iterating through all combos. Devide sum prob by the number of combos
		fullProbs[k] = prob / len(kjCombos)
	return fullProbs
...
def initialize(m,spectfile,xGrid,aGrid,MAF,nofreq,nosub):
    ...
    if nofreq:
        ...
        Fx={}
        for X in xGrid:
            p0 = getJKprob(m,N,X,0) #p0==pn
            pn = getJKprob(m,N,X,N)

"""BalLeRMix---Balancing seLection Likelihood Ratio based on Mixture models
Citation:
X Cheng, M DeGiorgio (2020) Flexible mixture model approaches that accommodate footprint size variability for robust detection of balancing selection. Molecular Biology and Evolution, 37(11): 3267--3291
"""
import argparse
import re
import sys
...
def getLogSpect(spectfile, nosub, MAF):
    """Read un-normalized neutral spect
...
def composition(mk, nk):
    """Generate collections of sorted 1D mk-long vectors
    :param mk: int Number of balanced alleles
    :param nk: Sample size 2n
    """
    global ComposSet
    if mk < 1 or nk < 0:
        return []
...
def getKprob(m, n, X, k, nofreq=False):
    ...
    :param m: Number of balanced alleles
    :param n: Sample size
    :param X: m-elelment vector of equilibrium frequencies
    :param k: Number of derived alleles
    :return:
    """
    xProbs = {}
    Xpool = {}
...
def getFullKprobs(m, n, X):
    """
    Compute full un-normalized probs
    :param m: Number of balanced alleles
    :param n: Sample size
    :param X: m-elelment vector of equilibrium frequencies
    :return:
...
def scan_fixSize_noCenter(xGrid, AGrid, AdGrid, aGrid, outfile, phys_pos, pos_list, Ks, Ns, numSites, Spec, logSpec, Fx,
                          N, Rrate, w, s, MAF=False):
    # w = float(r)
    ...
    with open(outfile, 'w') as scores:
        ...
        while midpos <= phys_pos[-1]:
            ...
            if start_i >= end_i:
                ...
                Tmax, optWinSize = calcBaller(winPosList, WinKs, WinNs, midpos * Rrate, pos_i - start_i, logSpec, Fx,
                                              xGrid, AGrid, AdGrid, aGrid, MAF)
...
def scan_fixSize_siteCenter(xGrid, AGrid, AdGrid, aGrid, outfile, phys_pos, pos_list, Ks, Ns, numSites, Spec, logSpec,
                            Fx, N, Rrate, w, s, MAF=False):
    ...
    with open(outfile, 'w') as scores:
        ...
        while i < numSites:
            testSite = phys_pos[i]
            start = max(0, testSite - w / 2)
            end = min(testSite + w / 2, phys_pos[-1])
            while phys_pos[start_i] < start:
                start_i += 1
            while end_i < numSites:
                if phys_pos[end_i] < end:
            ...
            # Window = Ks[start_i:end_i+1]
            Tmax, optWinSize = calcBaller(pos_list[start_i:end_i + 1], Ks[start_i:end_i + 1], Ns[start_i:end_i + 1],
                                          pos_list[i], i - start_i, logSpec, Fx, xGrid, AGrid, AdGrid, aGrid, MAF)
...
def scan_siteBased(xGrid, AGrid, AdGrid, aGrid, outfile, phys_pos, pos_list, Ks, Ns, numSites, Spec, logSpec, Fx, N,
    ...
    with open(outfile, 'w') as scores:
        ...
        while i < numSites:
            testSite = pos_list[i]
            start_i = max(0, i - r)
            end_i = min(numSites, i + r + 1)
            Tmax, optWinSize = calcBaller(pos_list[start_i:end_i], Ks[start_i:end_i], Ns[start_i:end_i], testSite,
                                          i - start_i, logSpec, Fx, xGrid, AGrid, AdGrid, aGrid, MAF)
            scores.write('%s\t%s\t%s\t%s\t%g\t%d\n' % (
                phys_pos[i], pos_list[i], Tmax[0], ','.join(['%g' % (_) for _ in Tmax[1]]), Tmax[2], optWinSize))
            i += s
    scores.close()
...
def scan_alpha(xGrid, AGrid, AdGrid, aGrid, outfile, phys_pos, pos_list, Ks, Ns, numSites, Spec, logSpec, Fx, N, s=1,
               MAF=False):
    """Perform the scan with size-variable window, varying alpha to change mixing weights"""
    print("writing output to %s" % (outfile))
    with open(outfile, 'w') as scores:
        scores.write('physPos\tgenPos\tLR\txhat\tAhat\tnumSites\n')
        i = 0
...
def scan(xGrid, AGrid, AdGrid, aGrid, outfile, phys_pos, pos_list, Ks, Ns, numSites, Spec, logSpec, Fx, N, size=False,
    ...
    if size:
        ...
        if not phys:
            print(
                'Please make sure to use physical positions as coordinates if fixed-length windows are chosen. Scan will continue with physical positions.')
            phys = True
            # sys.exit()
        if noCenter:
            w = float(r)
            print('Computing LR on %.3f kb windows on every %s bp. Using physical positions by default.' % (w / 1e3, s))
            scan_fixSize_noCenter(xGrid, AGrid, AdGrid, aGrid, outfile, phys_pos, pos_list, Ks, Ns, numSites, Spec,
                                  logSpec, Fx, N, Rrate, w, s, MAF)
        else:  # site-centered
            w = float(r)
            print(
                'Computing LR on %.2f kb windows on every %g informative sites. Using physical positions by default.' % (
                    w / 1e3, s))
            scan_fixSize_siteCenter(xGrid, AGrid, AdGrid, aGrid, outfile, phys_pos, pos_list, Ks, Ns, numSites, Spec,
                                    logSpec, Fx, N, Rrate, w, s, MAF)
    # When fixSize == False, and radius (-r) provided. Scan with fixed number of sites
        ...
        scan_siteBased(xGrid, AGrid, AdGrid, aGrid, outfile, phys_pos, pos_list, Ks, Ns, numSites, Spec, logSpec, Fx, N,
                       phys, Rrate, r, s, MAF)
...
def getConfig(infile, configfile):
    ...
    with open(infile, 'r') as sites:
        ...
        for l in sites:
            ...
            if x == 0:
                print(
                    'Please make sure the input has derived allele frequency. Sites with 0 observed allele count (k=0) will be ignored.\n')
                continue
            if n not in Config:
                Config[n] = [0, 0]
            Config[n][0] += int(x == n)
...
def main():
    ...
    parser.add_argument('--spect', dest='spectfile',
                        help='Path and name of the allele frequency spectrum file or configuration file.\n',
                        required=True)
    ...
    parser.add_argument('--getSpect', dest='getSpec', action='store_true', default=False,
                        help='Option to generate frequency spectrum file from the concatenated input file. Use \"-i\" and \"--spect\" commands to provide names and paths to input and output files, respectively. Indicate the input type with \"--MAF\" and/or \"--nosub\".\n')
    parser.add_argument('--getConfig', dest='getConfig', action='store_true', default=False,
                        help='Option to generate configuration file from the concatenated input file. Use \"-i\" and \"--spect\" commands to provide names and paths to input and output files, respectively.\n')

    parser.add_argument('--nofreq', dest='nofreq', action='store_true', default=False,
                        help='Option to ignore allele frequency information. All polymorphic sites will be considered as equivalent.')
    parser.add_argument('--nosub', dest='nosub', action='store_true', default=False,
                        help='Option to not include substitution in input data.')
    parser.add_argument('--MAF', dest='MAF', action='store_true', default=False,
                        help='Option to use minor allele frequency, instead of polarized allele frequency. The latter is default.')
    parser.add_argument('--physPos', action='store_true', dest='phys', default=False,
                        help='Option to use physical positions instead of genetic positions (in cM). Default is using genetic positions.\n')
    parser.add_argument('--rec', dest='Rrate', type=float, default=1e-6,
                        help='The uniform recombination rate in cM/nt. Default value is 1e-6 cM/nt. Only useful when choose to use physical positions as coordinates.\n\n')

    parser.add_argument('--fixSize', action='store_true', dest='size', default=False,
                        help='Option to fix the size of scanning windows. When true, provide the length of window in neucleotide (nt) with \"-w\" or \"--window\" command.\n')
    parser.add_argument('-w', '--window', dest='r', type=int, default=0,
                        help='Number of sites flanking the test locus on either side. When choose to fix window size (\"--fixSize\"), input the length of window in bp.\n')
    parser.add_argument('--noCenter', action='store_true', dest='noCenter', default=False,
                        help='Option to have the scanning windows not centered on informative sites. Require that the window size (\"-w\") in physical positions (\"--physPos\") is provided. Default is True.\n')
    parser.add_argument('-s', '--step', dest='step', type=float, default=1,
                        help='Step size in bp (when using \"--noCenter\") or the number of informative sites. Default value is one site or one nucleotide.\n\n')

    parser.add_argument('--fixX', dest='x',
                        help='Option to fix the presumed equilibrium frequency. When m>2, use the format \"(x1|x2|...|xm),(x1|x2|...|xm),...\" with each set of x1, x2,..., xm presented in descending order and summed up to one. \n')
    parser.add_argument('--rangeA', dest='seqA',
                        help='Range of the values of the parameter A to optimize over. Format should follow <Amin>,<Amax>,<Astep> with no space around commas.\n')
    parser.add_argument('--listA', dest='listA',
                        help='Manually provide a list of A values to optimize over. Please separate the values with comma, no space.\n')
    ...
    scan(xGrid, AGrid, AdGrid, aGrid, opt.outfile, phys_pos, pos_list, Ks, Ns, numSites, Spec, logSpec, Fx, N, opt.size,
         opt.r, opt.step, opt.phys, opt.nofreq, opt.MAF, opt.noCenter, opt.Rrate)  # , ProbsStar
